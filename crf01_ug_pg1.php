...
//qid 63
foreach($override->get('forms','qid',63) as $fid){//echo$fr.'  , ';
    $dbv=variable($fid['fid']);$am=null;
    $text=$override->get('formboxverifytext','fid',$fid['fid']);
    if($text){$desc=$text[0]['val'];}else{$desc='';}
    $am=null;$stf='';
    //print_r($dbv);
    $arr = array('country','institution','facility','tbenum','hospnum','vdate','clinic','age','marital','gender','occupation','education','location','hivpos','hivposyr','hivres','onart','onartyr','tbcasecontact','chronicillness','chronicillnessdx','alcohol','alcoholpres','tobacco','tobaccopres','drug','drugpres','tbtx','tbtxyr','tbsx01','tbsx02','tbsx01days','tbsx02days');
    ...
    try {//print_r( $override->getValueT($fid['fid']));echo ' , ';
        ...
        //print_r($am['drug']);echo' , ';
        if($override->selectData4('crf01_ug_pg01','country',$am['country'],'institution',$am['institution'],'facility',$am['facility'],'tbenum',preg_replace('/[^A-Za-z0-9\-]/', '', $am['tbenum']))){$dup=true;}else{$dup=false;}//echo$f.' , ';$f++;
        //print_r($am);echo '<br>';
        ...
        if($dbv && $dup==false){print_r($am);echo ' | ';
            $user->createRecord('crf01_ug_pg01', array(
                'country' => $cntry,
                'institution' => $inst,
                'facility' => $faci,
                'tbenum' => $tbnum,
