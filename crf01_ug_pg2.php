...
//qid = 64
foreach($override->get('forms','qid',64) as $fid){//echo$fr.'  , ';
    $dbv=variable($fid['fid']);$am=null;
    $text=$override->get('formboxverifytext','fid',$fid['fid']);
    if($text){$desc=$text[0]['val'];}else{$desc='';}
    $am=null;$stf='';
    $arr = array('country','institution','facility','tbenum','tbsx03','tbsx03days','tbsx04','tbsx04days','tbsx05','tbsx05days','tbsx06','tbsx06days','cough_care','carefac','othercarefac','xpertsputum','xpertstool','smear','cxray','tbscore','cxtbcase','formdate');
    ...
    try {//print_r( $override->getValueT($fid['fid']));echo ' , ';
        ...
        if($dbv && $dup==false){
            //print_r($tbsx_other);
            $user->createRecord('crf01_ug_pg02', array(
                'country' => $cntry,
                'institution' => $inst,
                'facility' => $faci,
                'tbenum' => $tbnum,
